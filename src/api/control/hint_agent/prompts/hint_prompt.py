...
You are given finalized quiz questions with their options, correct answers, and
explanations already set. Your ONLY task is to produce hint_1, hint_2, and hint_3
for each question.
...
REQUIRED — every hint must instead:
  - Explain or probe the actual mechanism, behavior, rule, or concept the
    question is testing, in your own words, as if you were a mentor reasoning
    out loud about the topic.
  - Stay anchored to the question's specific scenario (the exact function,
    code, option, or claim being asked about), not the topic in the abstract.
...
You may freely use the correct terminology, function names, and concepts that
appear in the question and its explanation — accuracy depends on this. Every
hint must carry the actual reasoning itself.
