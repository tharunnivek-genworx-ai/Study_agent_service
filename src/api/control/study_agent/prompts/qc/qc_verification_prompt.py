...
① must_cover — one check per checklist item
   ...
   DERIVATION MANDATORY CHECK — apply this whenever the requirement OR depth_gate uses the words derive, prove, calculate, trace, or step-by-step:
   Before issuing any pass decision, verify the section contains formula_blocks with SEQUENTIAL ALGEBRAIC OR LOGICAL STEPS where each step follows from the previous using standard mathematical operations. The following situations ALWAYS force passed=false regardless of how the depth_gate prose is worded:
   ...
   - The depth_gate or requirement demands derivation, proof, or step-by-step calculation, but the section only states the final formula, rule, or result — or provides only computational code.
...
② content_accuracy — one check per claim you can evaluate with certainty
   ...
   - For chemical reactions: verify the reactants, intermediates, mechanistic sequence, and products are consistent with the reaction class. Check the actual chemistry, not just the labels or symbols.   - Is every named reaction, mechanism, or compound a real one? A confident, well-formatted reaction or formula that you cannot positively verify as real chemistry/physics/mathematics — including plausible-sounding but fabricated reagents, products, or mechanisms — FAILS, even if no other error is present.
...
- NEVER pass a must_cover check with a derive/prove/calculate/step-by-step requirement when the section contains only a formula statement, a one-sentence explanation, or Python code — no exceptions.
...
def build_user_message(
    ...
    parts.append(
        ...
        "For derive/prove/calculate requirements: confirm sequential algebraic steps exist in formula_blocks — a formula statement or Python code is not a derivation. "
