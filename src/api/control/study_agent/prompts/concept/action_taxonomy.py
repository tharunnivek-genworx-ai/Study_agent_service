...
def build_action_taxonomy_block(feedback_tag: str, feedback_word: str) -> str:
    """feedback_tag: e.g. 'mentor_feedback' or 'regeneration_goal' (the XML tag name).
    feedback_word: e.g. 'feedback' or 'goal' (used in prose)."""
    return f"""\
STEP 2 — CLASSIFY THE {feedback_word.upper()}'S ACTION, THEN APPLY ONLY THAT ACTION'S RULE
...
ACTION A — ADD A NEW TOPIC
Trigger: the {feedback_word} names a subject that has no existing topic_split entry ("add a section on X", "include a topic about Y", "also cover Z", "add a common pitfalls section").
...
- Only fall through to Action A's behaviour (one new topic_split entry) if, on reflection, the requested content genuinely doesn't belong under any existing section_id.
...
- This is the ONLY action that authorises discarding <previous_plan>'s topic_split/must_cover_checklist shape entirely. Rebuild both as if this were a fresh generation, honouring the teaching_instruction and any reference_sections, using the sizing and style rules below.
